...
export default function Dashboard({setChoice}) {
  ...
  return (
    <div className='overflow-hidden flex flex-col justify-start h-[100vh] w-[22%] bg-gray-50 border-gray-200 dark:bg-gray-900 shadow-md dark:shadow-gray-400'>
        ...
        <div className='md:text-xl text-lg font-semibold text-gray-800 dark:text-gray-50 pb-2 border-b-[1px] border-b-solid border-b-slate-500 dark:border-b-slate-300 flex md:flex-row flex-col justify-between cursor-pointer mt-6' onClick={()=>setShowUserOptions(!showUserOptions)}>
            <h2>Users</h2>
            <span className="text-md md:text-xl">
                {showUserOptions?"⬆️":"⬇️"}
            </span>
